...
namespace Streamish.Repositories
{
    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public List<Video> GetAllWithComments()
        {
            using (var conn = Connection)
            {
                ...
                SELECT v.Id AS VideoId, v.Title, v.Description, v.Url, 
                       ...
                       cup.[Name] AS CommentUserProfileName, cup.Email AS CommentUserProfileEmail, 
                       cup.ImageUrl AS CommentUserProfileImageUrl, cup.DateCreated AS CommentUserProfileDateCreated
                  FROM Video v 
                       JOIN UserProfile up ON v.UserProfileId = up.Id
        ...
        public List<Video> MadeOnOrAfter(DateTime date)
        {
            using (var conn = Connection)
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    cmd.CommandText = @"SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId,
                                up.Name, up.Email, up.DateCreated AS UserProfileDateCreated,
                                up.ImageUrl AS UserProfileImageUrl
                                FROM Video v 
                                JOIN UserProfile up ON v.UserProfileId = up.Id
                                WHERE v.DateCreated >= @dateString";
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Id AS VideoId, v.Title, v.[Description], v.[Url], v.DateCreated AS VideoCreationDate, v.UserProfileId,
                          up.DateCreated AS UserProfileCreationDate, up.Email, up.ImageUrl, up.[Name]
                          FROM Video v
                        ...
                        if (reader.Read())
                        {
                            video = new Video()
                            {
                                ...
                                UserProfile = new UserProfile()
                                {
                                    ...
                                    Name = DbUtils.GetString(reader, "Name"),
                                    Email = DbUtils.GetString(reader, "Email"),
                                    DateCreated = DbUtils.GetDateTime(reader, "UserProfileCreationDate")
                                }
                            };
